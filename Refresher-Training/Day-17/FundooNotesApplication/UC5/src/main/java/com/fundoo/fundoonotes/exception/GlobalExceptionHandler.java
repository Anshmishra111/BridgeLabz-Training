 ...
 *   - InvalidNoteStateException        → 400 (illegal state transition, e.g., pin a trashed note)
...
public class GlobalExceptionHandler {
    ...
    // ── No auth in SecurityContext (SecurityUtils guard) ──────────────────
    ...
    public ResponseEntity<Map<String, Object>> handleIllegalState(
        ...
        // SecurityUtils.getCurrentUser() throws this when there is no authenticated
        // user in the context — treat as 401 Unauthorized, not 500.
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(errorBody(
                HttpStatus.UNAUTHORIZED, ex.getMessage(), request.getRequestURI(), null));
    }
